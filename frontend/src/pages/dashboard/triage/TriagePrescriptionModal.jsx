...
const TriagePrescriptionModal = ({ isOpen, onClose, record, onSuccess }) => {
    ...
    // Tab and history states for clinical timeline optimization
    const [historyTab, setHistoryTab] = useState('tests'); // 'tests' | 'emr'
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            try {
                const response = await api.get(`/doctor/patient/${pId}/history`);
                ...
                <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-12">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                        ...
                        <div className="lg:col-span-5 space-y-8 animate-fade-in">
                            {/* Tab Selection */}
                            <div className="flex items-center p-1.5 bg-slate-100/80 rounded-2xl border border-slate-200/60 z-10 shrink-0">
                                <button
                                    type="button"
                                ...
                                >
                                    ...
                                    <span>Tests & Advice</span>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => setHistoryTab('emr')}
                                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-black transition-all ${
                                        historyTab === 'emr'
                                            ? 'bg-white text-brand-dark shadow-md scale-105'
                                            : 'text-slate-500 hover:text-brand-dark'
                                ...
                                >
                                    ...
                                    {historyData?.timeline?.length > 0 && (
                                        <span className="px-2 py-0.5 bg-brand-teal text-white text-[9px] font-black rounded-full leading-none">
                            ...
                            {historyTab === 'tests' ? (
                                ...
                                <div className="space-y-6 max-h-[calc(100vh-16rem)] overflow-y-auto custom-scrollbar pr-2 pb-6">
                                    {historyLoading ? (
                                        <div className="flex flex-col items-center justify-center py-16 gap-3">
                                            <div className="w-10 h-10 border-4 border-brand-teal/20 border-t-brand-teal rounded-full animate-spin"></div>
                                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest animate-pulse">Retrieving EMR dossier...</p>
                                        </div>
                                    ) : !historyData ? (
                                        <div className="text-center py-12 bg-white rounded-[2rem] border border-slate-100 p-8 space-y-3 shadow-premium">
                                            <AlertCircle className="h-10 w-10 text-slate-300 mx-auto" />
                                            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">No patient profile resolved</p>
                                            <p className="text-[11px] text-slate-500 font-medium">This patient is currently unregistered. No historical clinical timelines detected.</p>
                                        </div>
                                    ) : (
                                        <>
                                            {/* returning patient intelligence summary */}
                                            <div className="bg-white rounded-[2rem] p-6 border border-slate-200/80 shadow-premium space-y-4">
                                                <h4 className="text-[10px] font-black text-brand-dark uppercase tracking-widest flex items-center gap-1.5">
                                                    <Sparkles className="h-3.5 w-3.5 text-brand-teal animate-pulse" /> Returning Patient Intelligence
                                                </h4>

                                                {/* allergies & medications info */}
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
                                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Allergies</p>
                                                        <p className="text-xs font-bold text-slate-800">
                                                            {historyData.patient?.allergies?.length > 0 
                                                                ? historyData.patient.allergies.join(', ') 
                                                                : 'None Recorded'}
                                                        </p>
                                                    </div>
                                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
                                                        ...
                                                        <p className="text-xs font-bold text-slate-800">
                                                            {historyData.patient?.medicalHistory?.length > 0
                                                                ? historyData.patient.medicalHistory.map(h => h.condition).join(', ')
                                                                : 'None Recorded'}
                                                        </p>
                                                    </div>
                                                </div>

                                                {/* automatic recall last prescription */}
                                                {historyData.lastPrescriptionWithDoctor && (
                                                    <div className="p-4 bg-brand-teal/10 border border-brand-teal/20 text-teal-950 rounded-xl space-y-1.5">
                                                        <p className="text-[9px] font-black text-brand-teal uppercase tracking-widest">Your Last Encounter Recall</p>
                                                        <p className="text-xs font-bold">
                                                            Diagnosed: <span className="text-brand-dark font-black">{historyData.lastPrescriptionWithDoctor.diagnosis}</span>
                                                        </p>
                                                        {historyData.lastPrescriptionWithDoctor.advice && (
                                                            <p className="text-[10px] font-medium text-slate-600 italic">
                                                                Advice: "{historyData.lastPrescriptionWithDoctor.advice}"
                                                            </p>
                                                        )}
                                                        <p className="text-[9px] font-mono text-slate-400 mt-1">
                                                            Issued on {new Date(historyData.lastPrescriptionWithDoctor.createdAt).toLocaleDateString()}
                                                        </p>
                                                    </div>
                                                )}

                                                {/* missed follow-ups */}
                                                {historyData.missedFollowUps?.length > 0 && (
                                                    <div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-950 rounded-xl flex items-start gap-2.5">
                                                        <AlertCircle className="h-4 w-4 text-rose-600 shrink-0 mt-0.5" />
                                                        <div>
                                                            <p className="text-[9px] font-black text-rose-800 uppercase tracking-widest">Missed Follow-Ups Detected</p>
                                                            <p className="text-xs font-bold text-rose-950">
                                                                Missed consultation on {new Date(historyData.missedFollowUps[0].date).toLocaleDateString()}
                                                            </p>
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {/* vitals trend gauge */}
                                                {historyData.vitalsHistory?.length > 0 && (
                                                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
                                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Vitals Comparison (Current vs. Last Intake)</p>
                                                        <div className="grid grid-cols-2 gap-4 text-xs font-bold">
                                                            <div>
                                                                <span className="text-[10px] text-slate-400 font-semibold block">Body Temp:</span>
                                                                <span className="text-brand-dark">{record.vitals?.temperature || '--'}°F </span>
                                                                <span className="text-[10px] font-normal text-slate-500">
                                                                    (prev: {historyData.vitalsHistory[0]?.temperature || 'N/A'}°F)
                                                                </span>
                                                            </div>
                                                            <div>
                                                                <span className="text-[10px] text-slate-400 font-semibold block">Blood Pressure:</span>
                                                                <span className="text-brand-dark">{record.vitals?.bloodPressure || record.vitals?.bp || '--'}</span>
                                                                <span className="text-[10px] font-normal text-slate-500">
                                                                    (prev: {historyData.vitalsHistory[0]?.bloodPressure || 'N/A'})
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="relative pl-6 border-l-2 border-slate-200/80 space-y-6 ml-2 pt-2">
                                                {historyData.timeline.map((event, idx) => (
                                                    <div key={idx} className="relative group">
                                                        {/* Icon pointer */}
                                                        <div className="absolute -left-[35px] top-1.5 h-[22px] w-[22px] rounded-full border border-slate-200 bg-white shadow-sm flex items-center justify-center text-brand-teal group-hover:scale-110 transition-transform">
                                                            <div className="h-1.5 w-1.5 rounded-full bg-brand-teal"></div>
                                                        </div>

                                                        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 hover:border-brand-teal/30 hover:shadow-md transition-all text-xs font-bold space-y-2">
                                                            <div className="flex items-center justify-between text-[9px] text-slate-400 font-mono uppercase tracking-wider">
                                                                <span>{new Date(event.date).toLocaleDateString()}</span>
                                                                <span className="px-2 py-0.5 bg-brand-light text-brand-teal border border-brand-teal/10 rounded-full text-[8px] font-black uppercase">
                                                                    {event.type}
                                                                </span>
                                                            </div>
                                                            <h5 className="text-sm font-black text-brand-dark font-display truncate">{event.title}</h5>
                                                            <p className="text-[10px] text-slate-500 font-medium">{event.doctor} • <span className="italic">{event.facility}</span></p>
                                                            
                                                            {/* Additional drop-down details for timeline */}
                                                            {event.details && (
                                                                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 mt-2 space-y-1.5 text-[11px] font-bold text-slate-700">
                                                                    {event.type === 'prescription' && (
                                                                        <>
                                                                            <p className="text-brand-teal">💊 Prescribed Medicines:</p>
